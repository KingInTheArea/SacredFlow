...
There is also **separate** **README file for DM count** which is kept same without any changes so that any other user can know all bout DM count framework we borrowed from official repo.
This codebase also attempts to calculate mean time for a person to get out of camera feed which can be implemented in finding **average time taken** by a **devotee** to get **Darshan**.
